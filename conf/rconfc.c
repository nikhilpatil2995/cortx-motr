 ...
 *
 * The first stage of rconfc startup is determining the entry point of motr
...
static int _confc_cache_clean(struct m0_confc *confc)
{
	 ...
	 * reelection, so let's reinitialise root object in different way.
	 */
				 ...
				 struct m0_sm_ast   *ast)
{
	...
	if (lnk->rl_state != CONFC_DEAD && !lnk->rl_fom_queued) {
		 ...
		 * implemented in the context of MOTR-2113,2150.
		 */
	} else {
			       ...
			       struct m0_confc *confc)
{
	...
	for (idx = 0; idx < va->va_count; idx++) {
		M0_LOG(M0_DEBUG, "idx = %d vi_count = %"PRIu32" quorum = %"PRIu32, 
			                                        idx, va->va_items[idx].vi_count, rconfc->rc_quorum);
